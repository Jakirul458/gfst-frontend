...
function UpdateInvestmentAcc() {
  ...
  useEffect(() => {
    const fetchAccountDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/investment/${accountNo}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:3001/api/investment/${accountNo}`, accountDetails);
