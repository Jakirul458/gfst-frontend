...
const Profit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        console.error('Error during profit transaction:', err);
...
const handleVerify = async () => {
    try {
        ...
        if (err.response) {
            setError(err.response.data.message || 'Server error while verifying account.');
        } else if (err.request) {
            setError('No response from server. Please check your internet connection.');
        } else {
            setError('An unexpected error occurred while verifying account.');
        }
